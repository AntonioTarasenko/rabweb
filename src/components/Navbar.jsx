                    ...
                    {navLinks.map((link, index) => (
                        <li key={index} className="relative group">
                            ...
                            onClick={() => link.dropdown && toggleDropdown(index)}>
                                {link.text}
                                {link.icon && (
                                    <span className={`transition-transform ${openDropdown === index ? 'rotate-180' : ''}`}>
                                        {link.icon}
                                    </span>
                                )}
                            </div>
                            {link.dropdown && (
                                <div className={`absolute top-full left-0 mt-2 w-48 bg-gray-800 rounded-md shadow-lg py-1 z-50 transition-all duration-300 
                ...
                <div className="flex items-center gap-4">
                    <div className="relative hidden md:block">
                        <input
                         type="text" 
                         placeholder="Search"
                         className="bg-gray-800 border-2 border-blue-100 text-sm text-white pl-8 pr-3 py-1 rounded-md placeholder:text-gray-400 focus:outline-none"
                         />
                ...
                <div className="md:hidden bg-gray-900 text-white px-6 py-4">
                    <div className="relative mb-4">
                        <input
                         type="text" 
                         placeholder="Search ..."
                         className="w-full bg-gray-800 border-2 border-blue-100 text-sm text-white pl-8 pr-3 py-2 rounded-md placeholder:text-gray-400 focus:outline-none"
                         />
                         <FiSearch className="absolute left-2 top-1/2 transform -translate-y-1/2 text-gray-400 text-sm" />
                    </div>
                    <ul className="space-y-3">
                        {navLinks.map((link, index) => (
                            <li key={index}>
                                <div className="flex items-center justify-between py-2 border-b border-gray-700 cursor-pointer hover:text-gray-300"
                                onClick={() => link.dropdown && toggleDropdown(index)}>
                                    <span>
                                        {link.text}
                                    </span>
